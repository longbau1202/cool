...
@section('content')
    ...
    <div class="d-flex mb-2 justify-content-between">
        ...
        <div class="d-flex justify-content-end">
            ...
            <button type="reset" form="addform" class="btn btn-secondary ml-2"><i class="uil-refresh mr-1"></i>Clear
            </button>
        </div>
    </div>
    <div class="row">
        <div class="col-12">
            <form id="addform" name="addform" action="{{ route('maker.store') }}" method="post"
                            ...
                            <div class="col-xl-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name" name="name"
                                           ...
                                           placeholder="Enter Maker name" value="{{ old('name') }}">
                                </div>
                                @error('name')
                                <span style="color:red;">{{$message}}</span>
                                @enderror
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" data-toggle="select2" name="country">
                                        @foreach(Config::get("common.COUNTRY") as $key => $value)
                                            <option
                                                value="{{ $value }}" {{ old('country') == $value ? 'selected': false }} >{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="link" name="link"
                                           ...
                                           placeholder="Enter Link page" value="{{ old('link') }}">
                                </div>
                                @error('link')
                                <span style="color:red;">{{$message}}</span>
                                @enderror
